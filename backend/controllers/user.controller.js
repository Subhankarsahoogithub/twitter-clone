...
export const followUnfollowUser=async(req,res)=>{
    try {
        //fetch the id paremeter:
        const {id}=req.params;
        //find the current user and the user tobe followed:
        const userToModify=await User.findById(id);
        const currentUser=await User.findById(req.user._id);
        //request to follow it'self:
        if(id === req.user._id.toString()){
            return res.status(400).json({error:"you cann't follow/unfollow yourself..."})
        }
        //one or both users not exist:
        if(!userToModify || !currentUser){
            return res.status(404).json({error:"user not found"})
        }
        //follow or unfollow the requested user-id:
        ...
        //2 cases possible:
           ...
           if(isFollowing){
              ...
              //unfollow sucessfully:
               //return the id of the user unfollowed by:
           ...
           //case2:
             ...
             //follow the user:
              ...
              //return the id of the user followed by:
              return res.status(200).json({message:"user followed sucessfully..."});
           }
        
    } catch (error) {
        console.log('error with followUnfollowUser handler function:',error.message);
        return res.status(500).json({error:error.message});
    }
}
...
export const updateUser=async(req,res)=>{
    ...
    try {
        ...
        if((currentPassword && !newPassword)||(!currentPassword && newPassword)) return res.status(400).json({error:"please provide both of the passwords..."})
        ...
        //if both passwords provided: updation is related to passwords:
          if(currentPassword && newPassword){
              ...
              if(!isMatch) return res.status(400).json({error:"provided password didn't match:"});
              //new password is invalid:
              if(newPassword.length < 6)return res.status(400).json({error:"new password must be of atleast 6 charecters:"})
